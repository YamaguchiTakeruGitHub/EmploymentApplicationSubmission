...
void JoyPad::Update()
{
	...
	//Bボタン
	if (pad.Buttons[PadButton::B] && !pisB)
	{
		pisB = true;
		isB = true;//別クラスで判定する用のフラグ
	}
	//Yボタン
	if (pad.Buttons[PadButton::Y] && !pisY)
	{
		pisY = true;
		isY = true;//別クラスで判定する用のフラグ
	}
	//Xボタン
	if (pad.Buttons[PadButton::X] && !pisX)
	{
		pisX = true;
		isX = true;//別クラスで判定する用のフラグ
	}
	//LBボタン
	if (pad.Buttons[PadButton::LB] && !pisLB)
	{
		pisLB = true;
		isLB = true;//別クラスで判定する用のフラグ
	}
	//RBボタン
	if (pad.Buttons[PadButton::RB] && !pisRB)
	{
		pisRB = true;
		isRB = true;//別クラスで判定する用のフラグ
	}
	//BACKボタン
	if (pad.Buttons[PadButton::BACK] && !pisBACK)
	{
		pisBACK = true;
		isBACK = true;//別クラスで判定する用のフラグ
	}
	//STARTボタン
	if (pad.Buttons[PadButton::START] && !pisSTART)
	{
		pisSTART = true;
		isSTART = true;//別クラスで判定する用のフラグ
	}
	//STICKLボタン
	if (pad.Buttons[PadButton::STICKBL] && !pisSTICKBL)
	{
		pisSTICKBL = true;
		isSTICKBL = true;//別クラスで判定する用のフラグ
	}
	//STICKRボタン
	if (pad.Buttons[PadButton::STICKBR] && !pisSTICKBR)
	{
		pisSTICKBR = true;
